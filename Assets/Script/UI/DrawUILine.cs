...
public class DrawUILine : MonoBehaviour
{
    ...
    public void DrawLine(Vector3 start, List<Vector3> path, bool isObstacle)
    {
        ...
        for (int i = 0; i < path.Count - 1; i++)
        {
            ...
            //    float lineWidth = Vector2.Distance(start, path[0]) * 1.2f;
    ...
    public bool checkLinelist(UILine uILine)
    {
        ...
        if(lineList[idx].GetComponent<UILine>().getId() == uILine.getId()) // 지금 순번이 맞다
        {
            idx++;
            return true;
        }
        ...
        {
            //Debug.Log($"No! {lineList[idx].GetComponent<UILine>().getId()} {uILine.getId()}");
            return false;
        }
    }
}
